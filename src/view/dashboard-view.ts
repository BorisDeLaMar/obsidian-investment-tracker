...
export class InvestmentDashboardView extends ItemView {
	...
	private async handleSberXlsxImport(file: File): Promise<void> {
		...
		try {
			const arrayBuffer = await this.readFileAsArrayBuffer(file);
			const transactions = this.plugin.parserDispatcher.parseSberXlsx(arrayBuffer);
			if (transactions.length === 0) {
	...
	public async updateDashboard(): Promise<void> {
		try {
			let transactions = await this.plugin.dataStore.getTransactions();
	...
	private renderSummaryCards(positions: Position[], transactions: Transaction[]): void {
		// Получаем сводку по брокерам
		const brokerSummaries = this.portfolioCalculator.calculateBrokerSummaries(transactions, positions);

		// 1. Общая сумма внесённых средств (чистый приток) – сумма по всем брокерам
		const totalInvested = brokerSummaries.reduce((sum, s) => sum + s.totalInvested, 0);

		// 2. Текущая стоимость портфеля = сумма currentValue по всем брокерам (активы + кэш)
		const totalCapital = brokerSummaries.reduce((sum, s) => sum + s.currentValue, 0);
		...
		// --- Карточка "Всего внесено средств" ---
		if (this.summaryInvestedValueEl) {
			...
			span.style.cssText = 'font-size: 22px; font-weight: 600; color: var(--text-normal);';
			const investedSummaries = brokerSummaries.map(s => ({ broker: s.broker, value: s.totalInvested }));
